    ...
    for (i = 0; i < mazeSize; i++) {
      for (j = 0; j < mazeSize-1; j++) {
        ...
        if (mazeArray[i][j+1] == 1) { 
          ...
          var wallCenter = new THREE.Vector3(origin.x + (j*cellSize) + (wallLength/2) - WALL_WIDTH/2, WALL_HEIGHT/2, origin.z + (i * cellSize) + (cellSize/2));
    ...
    for (j = 0; j < mazeSize; j++) {
      for (i = 0; i < mazeSize-1; i++) {
        ...
        if (mazeArray[i+1][j] == 1) { 
          ...
          var wallCenter = new THREE.Vector3(origin.x + (j*cellSize), WALL_HEIGHT/2, origin.z + (i * cellSize) + (cellSize/2) + (wallLength/2));
